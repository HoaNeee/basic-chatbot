...
export const promptTitle = (content: string) => {
  return `Hãy tạo cho tôi một tiêu đề đoạn chat ngắn gọn dựa vào input sau: ${content}. Tiêu đề nên súc tích, rõ ràng, chỉ trả về chuỗi thông thường, không cần giải thích gì thêm. `;
};
...
export const promptIntent = (content: string, messages: TMessage[]) => {
  return `
    Nhiệm vụ của bạn là phân tích intent dựa vào input sau: ${content},
    Danh sách các tin nhắn trước đó: ${JSON.stringify(
...
export const promptWeather = (
  ...
  return `
    ...
    Nếu dữ liệu không có, hoặc địa điểm có trong dữ liệu không khớp với input hãy hỏi lại địa điểm cụ thể hoặc địa điểm khác (trả về "content": câu trả lời của bạn yêu cầu người dùng cung cấp thêm thông tin).
